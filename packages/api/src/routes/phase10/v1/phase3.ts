...
import { hit, play } from '@/libs/phase10/phase3'
...
import { validateCard, validateId, validatePhase3 } from '@jgames/validations'
...
router.post('/hit', async (
  ...
  let commit = false
 const hitCount = added ? added.length : cards.length
...
router.post('/play', async (
  req: PostRequest<{ phase: Phase<3>, gameId: string, userId: string }>,
  res: Response<void>,
) => {
  const phase = validatePhase3(req.body.phase)
  const gameId = validateId(req.body.gameId)
  const userId = validateId(req.body.userId)

  for (const card of phase.set4) { validateCard(card) }
  for (const card of phase.run4) { validateCard(card) }
