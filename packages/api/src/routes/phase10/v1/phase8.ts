...
import { validateCard, validateId, validatePhase8 } from '@jgames/validations'
...
router.post('/play', async (
  ...
  const phase = validatePhase8(req.body.phase)
  const gameId = validateId(req.body.gameId)
  const userId = validateId(req.body.userId)

  for (const card of phase.color7) { validateCard(card) }
