...
import { hit, play } from '@/libs/phase10/phase5'
...
import { validateCard, validateId, validatePhase5 } from '@jgames/validations'
...
router.post('/hit', async (
  ...
  if (!req.body.added || !req.body.run8) {
    throw new RequestError('', 400)
  }

  const added: Card[] = req.body.added
  const cards: Card[] = req.body.run8
...
router.post('/play', async (
  req: PostRequest<{ phase: Phase<5>, gameId: string, userId: string }>,
  res: Response<void>,
) => {
  const phase = validatePhase5(req.body.phase)
  const gameId = validateId(req.body.gameId)
  const userId = validateId(req.body.userId)

  for (const card of phase.run8) { validateCard(card) }
