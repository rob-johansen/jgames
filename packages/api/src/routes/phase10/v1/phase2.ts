...
import { hit, play } from '@/libs/phase10/phase2'
...
import { validateCard, validateId, validatePhase2 } from '@jgames/validations'
...
router.post('/play', async (
  req: PostRequest<{ phase: Phase<2>, gameId: string, userId: string }>,
  res: Response<void>,
) => {
  const phase = validatePhase2(req.body.phase)
  const gameId = validateId(req.body.gameId)
  const userId = validateId(req.body.userId)

  for (const card of phase.set3) { validateCard(card) }
