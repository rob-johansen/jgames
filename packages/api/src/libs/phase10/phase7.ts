...
export const play = (phase: Phase<7>, userId: string, players: Player[]): boolean => {
  ...
  if (!player) {
    logger.error('Error playing phase 7 (player not found)')
    return false
  }

  const cards = player.cards as Card[]
  const phaseCards = phase.set4a.concat(phase.set4b)
  ...
  if (cards.length !== 3) {
    logger.error('Error playing phase 7 (card move problem)')
    return false
  }
