...
export const hit = ({ added, cards, hitteeId, hitterId, players }: HitProps): boolean => {
  ...
  if (!hitter) {
    logger.error('Error hitting phase 4 (hitter not found)')
    return false
  }

  const hittee = players.find((p) => p.id === hitteeId)
  if (!hittee) {
    logger.error('Error hitting phase 4 (hittee not found)')
    return false
  }

  const hitterCards = hitter.cards as Card[]
  const hitteeCards = (hittee.played as Phase<4>).run7
  ...
  if (added.length !== 0) {
    logger.error('Error hitting phase 4 (run7 move problem)')
    return false
  }
...
export const play = (phase: Phase<4>, userId: string, players: Player[]): boolean => {
  ...
  const phaseCards = structuredClone(phase.run7)
