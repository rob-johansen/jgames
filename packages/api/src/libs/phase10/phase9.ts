...
export const hit = ({ cards, hitteeId, hitterId, players, phasePart }: HitProps): boolean => {
  ...
  if (!hitter) {
    logger.error('Error hitting phase 9 (hitter not found)')
    return false
  }

  const hittee = players.find((p) => p.id === hitteeId)
  if (!hittee) {
    logger.error('Error hitting phase 9 (hittee not found)')
    return false
  }

  const hitterCards = hitter.cards as Card[]
  const hitteeCards = phasePart === 1 ? (hittee.played as Phase<9>).set5 : (hittee.played as Phase<9>).set2
  ...
  if (cards.length !== 0) {
    logger.error('Error hitting phase 9 (card move problem)')
    return false
  }
...
export const play = (phase: Phase<9>, userId: string, players: Player[]): boolean => {
  const player = players.find((p) => p.id === userId)
  if (!player) {
    logger.error('Error playing phase 9 (player not found)')
    return false
  }

  const cards = player.cards as Card[]
  const phaseCards = phase.set5.concat(phase.set2)
  ...
  if (cards.length !== 4) {
    logger.error('Error playing phase 9 (card move problem)')
    return false
  }
