...
export const hit = ({ added, cards, hitteeId, hitterId, players }: HitProps): boolean => {
  ...
  if (!hitter) {
    logger.error('Error hitting phase 6 (hitter not found)')
    return false
  }

  const hittee = players.find((p) => p.id === hitteeId)
  if (!hittee) {
    logger.error('Error hitting phase 6 (hittee not found)')
    return false
  }

  const hitterCards = hitter.cards as Card[]
  const hitteeCards = (hittee.played as Phase<6>).run9
  ...
  if (added.length !== 0) {
    logger.error('Error hitting phase 6 (run9 move problem)')
    return false
  }
