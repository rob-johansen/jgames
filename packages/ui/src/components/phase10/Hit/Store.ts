...
export class HitStore {
  ...
  onClickConfirm = async () => {
    ...
    if ((player.played as Phase<1>).set3a) {
      ...
      api = 'phase5'
      body = { added, run8: this.state.cards.map((c) => ({ color: c.color, value: c.value })) }
    } else if ((player.played as Phase<6>).run9) {
      api = 'phase6'
      body = { added, run9: this.state.cards.map((c) => ({ color: c.color, value: c.value })) }
    } else if ((player.played as Phase<7>).set4a) {
      api = 'phase7'
      ...
      api = 'phase9'
      body = this.state.phaseIndex === 0 ? { set5: added } : { set2: added }
    }
