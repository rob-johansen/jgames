...
export const Phase3 = observer(() => {
  const { phase3: store } = useContext(StoreContext)

  return (
    <div className="absolute border border-slate-300 shadow-phase h-fit left-0 mt-[40px] mx-auto pb-[32px] right-0 rounded-[8px] w-[848px]">
      <div className="flex gap-x-[24px]">
        <div className="border-b border-b-slate-300 border-r border-r-slate-300 flex h-[273px] items-center justify-center px-[24px] py-[20px] relative rounded-br-[8px] w-[358px]">
          ...
          {store.state.set.map((card, index) => {
            return (
              <Card
                ...
                inHand={false}
                key={card.id}
                onClick={() => store.onClickCard(card, 'set')}
              />
            )
          })}
        </div>
          ...
          {store.state.run.map((card, index) => {
            return (
              <Card
                ...
                inHand={false}
                key={card.id}
                onClick={() => store.onClickCard(card, 'run')}
              />
            )
          })}
        </div>
